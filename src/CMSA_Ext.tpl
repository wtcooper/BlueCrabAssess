...
DATA_SECTION
  ...
  init_int numBlocks
  init_vector blockYears(1,numBlocks)
  init_int blockProcess  //flag for which process to block (1=M, 2=sel)
  init_int linearBlock //linear relations for the blocks
  ...
  vector blockStartIndex(1,numBlocks)
  vector blockEndIndex(1,numBlocks)
  int blockPhase

 LOCAL_CALCS
//only use log_block_dev parameter for M or selectivity, not for FIM q
  if (numBlocks>1 && blockProcess!=3) blockPhase=2;
  else blockPhase=-1;

  blockStartIndex(1)=startIndex;
  blockEndIndex(numBlocks)=endIndex; //set the last end index

  for (int block=2; block<=numBlocks; block++){
    blockStartIndex(block)=blockStartIndex(block-1)+(blockYears(block)-blockYears(block-1));
    if (block<numBlocks) blockEndIndex(block)=blockStartIndex(block+1)-1;
  }
  if (numBlocks>1) blockEndIndex(1)=blockStartIndex(2)-1; //set the 1st end index
 END_CALCS
  ...
  init_int timeStepFlag   //flag to tell if calendar based time step or modified
  ...
  //#################################
  init_int fzyear  //first year of total catch
  init_int lzyear  //last year of total catch
  int zStartIndex
  int zEndIndex
  int zTimeSteps  //catch time steps based on catch years
  !!zTimeSteps = (lzyear-fzyear+1)*timeSteps;
  !!zStartIndex = (fzyear-fyear)*timeSteps+startIndex;
  !!zEndIndex = zStartIndex+zTimeSteps;
  ...
  init_vector MFlag(1,stages)	//which stage is variable in either blocking or environment (0=off, 1=on)
...
//convert M to the appropriate time frame from per year basis
 ...
 END_CALCS
  ...
  init_int recFitType				//env influence on recruitment fit type (data or model method as per Schrippa et al 09) 
  init_int MFitType					//env influence on mortality fit type (data or model method as per Schrippa et al 09)
  matrix env(1,numEnvTS,eStartIndex,endIndex+projSteps+1) //add on one for the forward stepping recruitment

  //extra environment variables
  vector negEnvFlag(1,numEnvTS)
  ...
  //M params
  init_number M_devIni
  init_vector M_devParams(1,3)
  init_number M_cvIni
  ...
  init_number F0_lambda		//survey weight
  ...
  number F0_var  //variance of catch
...
//########## Set up Environment/Blocking for M or recruits ##########
  if (lambdaEstOn==0) lambdaPhase=-1;
  //make sure only one fitting approach for environment is turned on for recruitment,M 
  if (MFitType==0) { //environmental influence off
    M_devParams(3)=-1;			//turn off
    M_beta_envParams(3)=-1; 	//turn off
  }
  else if (MFitType==1) { //data type
    M_devParams(3)=3;
    M_beta_envParams(3)=3;
  }
  else if (MFitType==2) { //model type
    M_devParams(3)=-1;
    M_beta_envParams(3)=3;
  }

  if (recFitType==0 ) { //environmental influence off
    sr_beta_envParams(3)=-1; 	//turn off
  }
  else if (recFitType==1) { //data type
    sr_beta_envParams(3)=3;
  }
  else if (recFitType==2) { //model type
    sr_beta_envParams(3)=3;
  }

  //make sure the M blocking and environment are NOT turned on at same time
  if (M_beta_envParams(3)>0 && numBlocks>1 && blockProcess==1) {
    cout <<"Error: you have M blocks and environmental influence on M both turned on, choose just one. Now exiting..."<<endl;
    exit(1);
  }
  ...
  F0_var=log(F0_cv*F0_cv+1);  //variance of Z
  ...
  for (i=1; i<=numEnvTS; i++){
    ...
    //record whether the environment is negative valued, necessary for environment fit using 'data' method so not taking log of neg val
    if (min(env(i))<=0) negEnvFlag(i)=1;
    else negEnvFlag(i)=0;
  }
...
//template: double xxxMin=log(xxxParams(1)); double xxxMax=log(xxxParams(2)); double xxxPhase=log(xxxParams(2));
  ...
  double log_M_devMin=log(M_devParams(1)); 	double log_M_devMax=log(M_devParams(2));		double M_devPhase=M_devParams(3);
 ...
 END_CALCS
  ...
  //Natural mortality for each year
  init_bounded_dev_vector log_M1_dev(startIndex,endIndex,log_M_devMin,log_M_devMax,M_devPhase)
  init_bounded_dev_vector log_M2_dev(startIndex,endIndex,log_M_devMin,log_M_devMax,M_devPhase)
  ...
  //log of multiplicative deviation for block processes (M or selectivity)
  //Note: don't use bounded_dev because need to average to input across years, not blocks - use a penalty (block_pen)
  init_bounded_matrix log_block_dev(1,stages,1,numBlocks,-2,2,blockPhase)

  init_bounded_vector lambdaIndices(1,numAdSurv,0.0001,.9999,lambdaPhase)
  ...
  matrix selt(1,stages,startIndex,endIndex+projSteps)	//selectivity per time step
  ...
  matrix qa(1,numBlocks,1,numAdSurv)   //catchability for adult surveys 
  matrix qr(1,numBlocks,1,numRecSurv)   //catchability for recruitment surveys
  matrix qatemp(1,numAdSurv,startIndex,endIndex)
  matrix qrtemp(1,numRecSurv,startIndex,endIndex)
  number qslope
  number qint


  //temporary files for calendar year calculation of catch
  number NCalTemp					//hold for time step calcs
  vector TCCal(startIndex,endIndex+1+projSteps)      //total catch on calendar time step
  number ave
  number ave1
  number ave2
  ...
  number LF0        //likelihood components for initial F estimate   
  number Lrdev           //likelihood for recruitment deviations
  number Leff           //likelihood for effort residuals
  number LenvRec		//likelihood for environmental influence on recruitment if using data method
  number LenvM			//likelihood for environmental influence on M if using data method
 ...
 LOCAL_CALCS
  ...
  log_M1_dev=log(M_devIni);
  log_M2_dev=log(M_devIni);
  ...
  log_sel=log(selIni);
  for (i=1; i<=stages; i++) log_block_dev(i)=log(1.);
...
FUNCTION set_initial_conditions
  ...
  for (i=1; i<=stages; i++) {
    sel(i)=exp(log_sel(i)); //average
    selt(i)=exp(log_sel(i)); //year-specific set to average as default
  }
  ...
  if (lambdaEstOn==1) {
  	negLL+=1000000*(square(1.0-sum(lambdaIndices)));
  	if (testing==1) cout << "lambdaIndices: " << lambdaIndices << endl;
  	}
  ...
  //######## selectivity blocks ########  
  if (numBlocks>1 && blockProcess==2)  {
    ave=0.0;
    for (int block=1; block<=numBlocks; block++) {
      for(y=blockStartIndex(block); y<=blockEndIndex(block); y++) {
        selt(1,y)=sel(1)*exp(log_block_dev(1,block));
        ave=ave+selt(1,y);
      }
    }

    //add penalty if deviations across years don't average the input value (instead of using dev_vector)
    ave=ave/(endIndex-startIndex+1);
    negLL+=10000.*((sel(1)-ave)*(sel(1)-ave));

    //set projection equal to final year
    for (y=endIndex+1; y<=endIndex+projSteps; y++) {
      selt(1,y)=selt(1,endIndex);
    }
  }
  ...
  //if using blocked M's
  if (numBlocks>1 && blockProcess==1)  {
    ave1=0.0;
    ave2=0.0;
    for (int block=1; block<=numBlocks; block++) {
      for(y=blockStartIndex(block); y<=blockEndIndex(block); y++) {
        if (MFlag(1)==1) Mt(1,y)=M(1)*exp(log_block_dev(1,block));
        else Mt(1,y)=M(1);
        ave1=ave1+Mt(1,y);

        if (MFlag(2)==1) Mt(2,y)=M(2)*exp(log_block_dev(2,block));
        else Mt(2,y)=M(2);
        ave2=ave2+Mt(2,y);
    ...
    //add penalty if deviations across years don't average the input value
    ave1=ave1/(endIndex-startIndex+1);
    negLL+=10000.*((M(1)-ave1)*(M(1)-ave1));
    ave2=ave2/(endIndex-startIndex+1);
    negLL+=10000.*((M(2)-ave2)*(M(2)-ave2));

    //set projection equal to final year
    for (y=endIndex+1; y<=endIndex+projSteps; y++) {
      Mt(1,y)=Mt(1,endIndex);
      Mt(2,y)=Mt(2,endIndex);
    }
  }

  //NOT using blocked M's, but potentially environmental influence (only 1 or the other)
  else {
    ...
    for(y=startIndex; y<=endIndex+projSteps; y++) {
      ...
      Mt(1,y)=M(1);
      if (MFlag(1)==1 && MFitType==2) Mt(1,y)=M(1)*exp(M_beta_env(1)*env(envMTS(1),y-envMLag(1)))*exp(-biasAdj*0.5*M_var);

      if (y<=endIndex) {
        if (MFlag(1)==1 && MFitType==1)  Mt(1,y)=M(1)*exp(log_M1_dev(y))*exp(-biasAdj*0.5*M_var);
        posfun(maxM-(timeSteps*Mt(1,y)),.000001,M_pen);
        negLL+=10000.*M_pen;
      }

      Mt(2,y)=M(2);
      if (MFlag(2)==1 && MFitType==2) Mt(2,y)=M(2)*exp(M_beta_env(2)*env(envMTS(2),y-envMLag(2)))*exp(-biasAdj*0.5*M_var);

      if (y<=endIndex) {
        if (MFlag(2)==1 && MFitType==1)  Mt(2,y)=M(2)*exp(log_M2_dev(y))*exp(-biasAdj*0.5*M_var);
        posfun(maxM-(timeSteps*Mt(2,y)),.000001,M_pen);
        negLL+=10000.*M_pen;
      }
    }
...
FUNCTION calculate_abundance_and_catch
  ...
  TCCal(startIndex)=0.0;

  for(y=startIndex; y<=endIndex+projSteps; y++) {

    //spawners also include some animals that were recruits in the beginning of the year
    SP(y)=sratio*(N(y)*exp(-sp_time*(Mt(2,y)+selt(2,y)*F(y))))*pSpawn(2)
      + sratio*(R(y)*exp(-sp_time*(Mt(1,y)+selt(1,y)*F(y))))*pSpawn(1);

    if (SRSwitch==1)	{ //Beverton-Holt
      // don't use recruit deviations for projection years
      if (recFitType==2){ //model method of environmental control
      ...
      else { //environment either off or using data method
        if (y<endIndex) R(y+1)=(SP(y)/(SP(y)*beta+alpha))*exp(log_rec_dev(y+1)-biasAdj*0.5*rec_var);
        else R(y+1)=(SP(y)/(SP(y)*beta+alpha));
    ...
    if (SRSwitch==2)	{ //Ricker
      ...
      if (recFitType==2){
        ...
        if (y<endIndex) R(y+1)=(alpha*SP(y)*exp(-beta*SP(y)))*exp(log_rec_dev(y+1)-biasAdj*0.5*rec_var);
        else R(y+1)=(alpha*SP(y)*exp(-beta*SP(y)));

      }
    }

    //abundance for the next year
    N(y+1)=R(y)*exp(-(Mt(1,y)+selt(1,y)*F(y)))+N(y)*exp(-(Mt(2,y)+selt(2,y)*F(y)));

    //Baranov catch equation
    TC(y)=N(y)*((selt(2,y)*F(y))/(selt(2,y)*F(y)+Mt(2,y))*(1.-exp(-(selt(2,y)*F(y)+Mt(2,y)))))
      + R(y)*((selt(1,y)*F(y))/(selt(1,y)*F(y)+Mt(1,y))*(1.-exp(-(selt(1,y)*F(y)+Mt(1,y)))));

    u(y)=TC(y)/(R(y)*((1-exp(-selt(1,y)*F(y)))/(1-exp(-F(y))))+N(y));


    //###############################    
    // calculation for calendar year time step catch
    
    //do fall 
    //no selectivity on recruits during 1st half of time step, but full selectivity during 2nd half
    TCCal(y)=TCCal(y)+N(y)*((selt(2,y)*F(y)/2.0)/(selt(2,y)*F(y)/2.0+Mt(2,y)/2.0)*(1.-exp(-(selt(2,y)*F(y)/2.0+Mt(2,y)/2.0))));

    //do spring numbers
    NCalTemp=N(y)*exp(-(Mt(2,y)/2.0+selt(2,y)*F(y)/2.0));

    //add in all recruits from year since none were caught in spring and half the year adults
    TCCal(y+1)=NCalTemp*((selt(2,y)*F(y)/2.0)/(selt(2,y)*F(y)/2.0+Mt(2,y)/2.0)*(1.-exp(-(selt(2,y)*F(y)/2.0+Mt(2,y)/2.0))))
      + R(y)*((selt(1,y)*F(y))/(selt(1,y)*F(y)+Mt(1,y))*(1.-exp(-(selt(1,y)*F(y)+Mt(1,y)))));
...
FUNCTION calculate_predicted_indices
  ...
  if (numBlocks>1 && blockProcess==3)  {
        ...
        for(y=blockStartIndex(block); y<=blockEndIndex(block); y++) {
          ...
          if(re_survey_obs(i,y)!=-999.) { //check to make sure year is not missing
            if(!last_phase()) {
              ...
              qr(block,i)+=log(re_survey_obs(i,y))-log(R(y)*exp(-(sr_time(i)*Mt(1,y)))+.000001);
            }
            else { //small constant not included in last estimation stage
              qr(block,i)+=log(re_survey_obs(i,y))-log(R(y)*exp(-(sr_time(i)*Mt(1,y))));
            }
            counter++;
          }
        }
        //calculate geometric mean
        qr(block,i)=exp(qr(block,i)/counter);
        //Calculate predicted index of abundance
        //NOTE: this formulation assumes survey occurs before vulnerable to harvest
        if (linearBlock==0){
          for(y=blockStartIndex(block); y<=blockEndIndex(block); y++) {
            re_survey_est(i,y)=qr(block,i)*(R(y)*exp(-(sr_time(i)*Mt(1,y))));
          }
        }
      }
      ...
      for (i=1; i<=numAdSurv; i++){
        ...
        for(y=blockStartIndex(block); y<=blockEndIndex(block); y++) {
          ...
          if(ad_survey_obs(i,y)!=-999.) { //check to make sure year is not missing
            ...
            qa(block,i)+=log(ad_survey_obs(i,y))-log(N(y)*exp(-sa_time(i)*(Mt(2,y)+selt(2,y)*F(y))));
            counter++;
          }
        }
        //calculate geometric mean
        qa(block,i)=exp(qa(block,i)/counter);

        //Calculate each predicted index of abundance
        if (linearBlock==0){
          for(y=blockStartIndex(block); y<=blockEndIndex(block); y++) {
            ad_survey_est(i,y)=qa(block,i)*(N(y)*exp(-sa_time(i)*(Mt(2,y)+selt(2,y)*F(y))));
          }
        }
      }
    }
    ...
    if (linearBlock==1){
      ...
      for (i=1; i<=numRecSurv; i++){
        qslope=(qr(2,i)-qr(1,i))/(((blockEndIndex(2)-blockStartIndex(2))/2.+blockStartIndex(2))-((blockEndIndex(1)-blockStartIndex(1))/2.+blockStartIndex(1)));
        qint=qr(2,i)-qslope*((blockEndIndex(2)-blockStartIndex(2))/2.+blockStartIndex(2));
        for(y=startIndex; y<=endIndex; y++) {
          qrtemp(i,y)=qslope*y+qint;
          re_survey_est(i,y)=(qslope*y+qint)*(R(y)*exp(-(sr_time(i)*Mt(1,y))));
          if (testing==1) {
            cout<<"re_survey_est("<<i<<"): "<< re_survey_est(i,y)<<endl;
          }
      ...
      for (i=1; i<=numAdSurv; i++){
        qslope=(qa(2,i)-qa(1,i))/(((blockEndIndex(2)-blockStartIndex(2))/2.+blockStartIndex(2))-((blockEndIndex(1)-blockStartIndex(1))/2.+blockStartIndex(1)));
        qint=qa(2,i)-qslope*((blockEndIndex(2)-blockStartIndex(2))/2.+blockStartIndex(2));

        if (testing==1) {
          cout << "qslope " << qslope <<endl;
          cout << "qint " << qint <<endl;
          cout << "qa(2,i) "<< qa(2,i)<<endl;
          cout << "qa(1,i) "<< qa(1,i)<<endl;
          cout << "blockEndIndex(2) " << blockEndIndex(2)<<endl;
          cout << "blockStartIndex(2) " << blockStartIndex(2)<<endl;
          cout << "blockEndIndex(1) " << blockEndIndex(1)<<endl;
          cout << "blockStartIndex(1) " << blockStartIndex(1)<<endl;

        }

        for(y=startIndex; y<=endIndex; y++) {
          qatemp(i,y)=qslope*y+qint;
          ad_survey_est(i,y)=(qslope*y+qint)*(N(y)*exp(-sa_time(i)*(Mt(2,y)+selt(2,y)*F(y))));
          if (testing==1) {
            cout<<"ad_survey_est("<<i<<"): "<< ad_survey_est(i,y)<<endl;
          }

        }
      }
      if (testing==1) {
        cout<<"qrtemp (" <<i << "): "<< qrtemp(1)<<endl;
        cout<<"qatemp (" <<i << "): "<< qatemp(1)<<endl;
      }
    }
  }
      ...
      for(y=startIndex; y<=endIndex; y++) {
        ...
        if(re_survey_obs(i,y)!=-999.) { //check to make sure year is not missing
          if(!last_phase()) {
            ...
            //increase numerical stability
            //NOTE: this formulation assumes survey occurs before vulnerable to harvest
            qr(1,i)+=log(re_survey_obs(i,y))-log(R(y)*exp(-(sr_time(i)*Mt(1,y)))+.000001);
          }
          else { //small constant not included in last estimation stage
            qr(1,i)+=log(re_survey_obs(i,y))-log(R(y)*exp(-(sr_time(i)*Mt(1,y))));
          }
          counter++;
        }
      }
      //calculate geometric mean
      qr(1,i)=exp(qr(1,i)/counter);
      //Calculate predicted index of abundance
      //NOTE: this formulation assumes survey occurs before vulnerable to harvest
      for(y=startIndex; y<=endIndex; y++) {
        re_survey_est(i,y)=qr(1,i)*(R(y)*exp(-(sr_time(i)*Mt(1,y))));
      }
    }
    ...
    for (i=1; i<=numAdSurv; i++){
      ...
      for(y=startIndex; y<=endIndex; y++) {
        ...
        if(ad_survey_obs(i,y)!=-999.) { //check to make sure year is not missing
          ...
          qa(1,i)+=log(ad_survey_obs(i,y))-log(N(y)*exp(-sa_time(i)*(Mt(2,y)+selt(2,y)*F(y))));
          counter++;
        }
      }
      //calculate geometric mean
      qa(1,i)=exp(qa(1,i)/counter);

      //Calculate each predicted index of abundance
      for(y=startIndex; y<=endIndex; y++) {
        ad_survey_est(i,y)=qa(1,i)*(N(y)*exp(-sa_time(i)*(Mt(2,y)+selt(2,y)*F(y))));
      }
    }
  }
...
FUNCTION calculate_objective_function
  ...
  for (i=1; i<=numAdSurv; i++){
    ...
    if (lambdaEstOn==0) Lsa(i)=sa_lambda(i)*Lsa(i);
    if (lambdaEstOn==1) Lsa(i)=lambdaIndices(i)*Lsa(i);
  ...
  for (i=1; i<=numRecSurv; i++){
    ...
    if (lambdaEstOn==0) Lsr(i)=sr_lambda(i)*Lsr(i);
    if (lambdaEstOn==1) Lsr(i)=lambdaIndices(i)*Lsr(i);
  ...
  for(y=startIndex; y<=endIndex; y++) {
    ...
    if(TC_obs(y)!=-999.) { //check to make sure year is not missing
      if (timeStepFlag==1) Lc+=square(log(TC_obs(y)+.000001)-log(TC(y)+.000001));
      else if (timeStepFlag==0 && y>startIndex) Lc+=square(log(TC_obs(y)+.000001)-log(TCCal(y)+.000001)); //do calendar year
    }
  }
  ...
  Lz=aveZ_lambda*(0.5*log(2.*pi)+0.5*log(Z_var)+log(aveZ)+0.5*square(log(aveZ)-log(mean(Z(zStartIndex,zEndIndex-1))))/Z_var);


  //calculate likelihood components for environment-M link if using data method as per Schrippa et al. 2009
  LenvM=0.0;
  if (MFitType==1) {
    for(y=startIndex; y<=endIndex; y++) {
      if(MFlag(1)==1 && env(envMTS(1),y-envMLag(1))!=-999.) { //check to make sure year is not missing
        //note: if environment has negative values, have to subtract the minimum value to convert all to positve
        if (negEnvFlag(envMTS(1))==0) LenvM+=0.5*log(2.*pi)+0.5*log(M_var)+0.5*square(M_beta_env(1)*log(env(envMTS(1),y-envMLag(1)))-log(Mt(1,y)))/M_var;
        else LenvM+=0.5*log(2.*pi)+0.5*log(M_var)+0.5*square(M_beta_env(1)*log(env(envMTS(1),y-envMLag(1))-min(env(envMTS(1)))+1)-log(Mt(1,y)))/M_var;
      }
      if(MFlag(2)==1 && env(envMTS(2),y-envMLag(2))!=-999.) { //check to make sure year is not missing
        if (negEnvFlag(envMTS(2))==0) LenvM+=0.5*log(2.*pi)+0.5*log(M_var)+0.5*square(M_beta_env(2)*log(env(envMTS(2),y-envMLag(2)))-log(Mt(2,y)))/M_var;
        else LenvM+=0.5*log(2.*pi)+0.5*log(M_var)+0.5*square(M_beta_env(2)*log(env(envMTS(2),y-envMLag(2))-min(env(envMTS(2)))+1)-log(Mt(2,y)))/M_var;
      }
    }
  }

  //calculate likelihood components for environment-recruitment link if using data method as per Schrippa et al. 2009
  LenvRec=0.0;
  if (recFitType==1){
    for(y=startIndex; y<=endIndex; y++) {
      if(env(envRecTS,y+1-envRecLag)!=-999.) { //check to make sure year is not missing
        if (negEnvFlag(envRecTS)==0) LenvRec+=0.5*log(2.*pi)+0.5*log(rec_var)+0.5*square(sr_beta_env*log(env(envRecTS,y+1-envRecLag))-log_rec_dev(y))/rec_var;
        else LenvRec+=0.5*log(2.*pi)+0.5*log(rec_var)+0.5*square(sr_beta_env*log(env(envRecTS,y+1-envRecLag)-min(env(envRecTS))+1)-log_rec_dev(y))/rec_var;
      //cout << "LenvRec: " <<LenvRec << " , min: " << min(env(envRecTS))  << " , env: " << env(envRecTS,y+1-envRecLag)<<  " , recdev: " << log_rec_dev(y)<< endl;
      }
    }
  }
  ...
  //calculate likelihood component for total Z of adults as prior, read from independent Z estimate
  LF0=F0_lambda*(0.5*log(2.*pi)+0.5*log(F0_var)+log(F0)+0.5*square(log(F0)-log(F(startIndex)))/F0_var);


  negLL+=sum(Lsa)+sum(Lsr)+Lc+Lrdev+Leff+Lz+LenvRec+LenvM+LF0;
...
FUNCTION calculate_tSPR
  ...
  for(y=startIndex;y<=endIndex+projSteps;y++){
    ...
    SPR1=sratio*pSpawn(2)*exp(-(Mt(1,y)+selt(1,y)*F(y)+sp_time*(Mt(2,y)+selt(2,y)*F(y))))/(1.-exp(-(Mt(2,y)+selt(2,y)*F(y))))
      + sratio*pSpawn(1)*exp(-sp_time*(Mt(1,y)+selt(1,y)*F(y)));
...
FUNCTION obs_pred
  ...
  for(y=startIndex; y<=endIndex; y++) {
    ...
    //adult surveys
    for (i=1; i<=numAdSurv; i++) {
      if (y>=adStartIndex) ofs_op << i << " "<< year << " 0 a s 0 " << ad_survey_obs(i,y) << " " << ad_survey_est(i,y) << endl;
      if (y<adStartIndex) ofs_op << i << " "<< year << " 0 a s 0 " << "NA" << " " << ad_survey_est(i,y) << endl;
    }

    //recruit surveys
    for (i=1; i<=numRecSurv; i++) {
      if (y>=recStartIndex) ofs_op  << i << " "<< year << " 0 r s 0 " << re_survey_obs(i,y) << " " << re_survey_est(i,y) << endl;
      if (y<recStartIndex) ofs_op  << i << " "<< year << " 0 r s 0 " << "NA" << " " << re_survey_est(i,y) << endl;
    }
...
FUNCTION HPD_estimates
  ...
  ofs_hpd << "year Adult Spawners Rec RecSurvey1 TC TCCal recM adM  F sel1 sel2 FMSYRatio NMSYRatio FFLimRatio NNLimRatio u0 u1 uAll SREnv MEnvRec MEnvAd" << endl;

  year=fyear; //for outputting the year if multiple time steps per year

  for(y=startIndex;y<=endIndex+projSteps;y++){
    ofs_hpd << year << " " << N(y) << " "<< SP(y)<< " " << R(y) << " " <<R(y)*exp(-sr_time(1)*Mt(1,y)) << " " << TC(y) << " " << TCCal(y) << " " << Mt(1,y) << " " << Mt(2,y) << " " << F(y)<< " " << selt(1,y)<< " " << selt(2,y)<< " " << F(y)/FMSY<< " " << N(y)/NMSY<< " "<< F(y)/FLim<< " " << N(y)/NLim<< " "<< (selt(1,y)*F(y))/(selt(1,y)*F(y)+Mt(1,y))*(1.-exp(-(selt(1,y)*F(y)+Mt(1,y)))) << " "<< (selt(2,y)*F(y))/(selt(2,y)*F(y)+Mt(2,y))*(1.-exp(-(selt(2,y)*F(y)+Mt(2,y)))) << " "<< u(y) << " " << env(envRecTS,y-envRecLag) << " "<< env(envMTS(1),y-envMLag(1)) << " " << env(envMTS(2),y-envMLag(2)) << endl;
    year=year+1.0/timeSteps;
  }
...
FUNCTION general_report
  ofstream ofs_gen("gen_results.dat");
  {
    ...
    ofs_gen << "Lsa "<< mean(Lsa) <<endl;
    ofs_gen << "Lsr "<< mean(Lsr) << endl;
    ofs_gen << "Lc "<<  Lc << endl;
    ofs_gen << "Lz "<<  Lz << endl;
    ofs_gen << "LF0 "<<  LF0 << endl;
    ...
    for (int block=1; block<=numBlocks; block++){
      for (i=1; i<=numAdSurv; i++) ofs_gen << "qa_i"<<i << "_b" << block <<" " << qa(block,i) << endl;
      for (i=1; i<=numRecSurv; i++) ofs_gen << "qr_i"<<i << "_b" << block <<" " << qr(block,i) << endl;
    }
...
//Call reporting functions
  ...
  report << "LenvM\t" << LenvM<< endl;
  report << "LenvRec\t" << LenvRec<< endl;
  ...
  report << "sel1 "<< selt(1) << endl;
  report << "sel2 "<< selt(2) << endl;
  ...
  for (int block=1; block<=numBlocks; block++){
    for (i=1; i<=numAdSurv; i++) report << "qa_i"<<i << "_b" << block <<"\t" << qa(block,i) << endl;
    for (i=1; i<=numRecSurv; i++) report << "qr_i"<<i << "_b" << block <<"\t" << qr(block,i) << endl;
  }
  if (SRSwitch==1)	 //Beverton-Holt
  ...
  report << "lambdaIndices\t" << lambdaIndices <<endl;
